...
class Args:
    ...
    def __init__(self, config):
        ...
        self.set_cost_bbox = 5 #before 5
        self.set_cost_giou = 2  #before 2
        self.bbox_loss_coef = 5    #before 5
        self.giou_loss_coef = 2 #before 2
        self.eos_coef = 0.1
    ...
    for epoch in range(config["epochs"]):
        ...
        with torch.no_grad():
            for samples, targets in val_loader:
                ...
                # ===============================================
                # 🌟 NEW: METRICS CALCULATION (IoU & Accuracy) 🌟
                # ===============================================
                ...
                for i, (pred_idx, tgt_idx) in enumerate(indices):
                    ...
                    if len(pred_idx) > 0 and len(tgt_idx) > 0:
                        ...
                        # Find the predicted class ID (excluding the no-object logit)
                        # We take the top logit for the matched query (pred_idx[0])
                        ...
                        # 2. IoU Calculation
                        # Get the normalized box coordinates for the matched query and target
                        ...
                        if pred_boxes_normalized.numel() > 0:
                            ...
                            # Only take the IoU values for the matched pairs (diagonal)
...
def visualize_predictions(config):
    print("[Visualizer] Starting prediction visualization...")
